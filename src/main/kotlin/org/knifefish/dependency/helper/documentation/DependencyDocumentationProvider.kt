...
class DependencyDocumentationProvider : DocumentationProviderEx() {
    ...
    override fun getCustomDocumentationElement(
        ...
        val lookup = editor.getUserData(EDITOR_LOOKUP_KEY) ?: return null
        val range = lookup.dependency.inspectionRange
        if (targetOffset !in range.startOffset..range.endOffset) {
            return null
        }
        lookup.metadataPsi?.putUserData(LOOKUP_RESULT_KEY, lookup)
        lookup.originalElement.putUserData(LOOKUP_RESULT_KEY, lookup)
        return lookup.metadataPsi ?: lookup.originalElement
    }

    override fun generateDoc(element: PsiElement, originalElement: PsiElement?): String? {
        val result = element.getUserData(LOOKUP_RESULT_KEY) ?: originalElement?.getUserData(LOOKUP_RESULT_KEY) ?: return null
        val latestRule = originalElement?.project?.dependencyInsightService()?.latestVersionPolicy()?.displayName
            ?: result.latestRule
    ...
    companion object {
        ...
        fun showQuickDoc(editor: Editor, result: DependencyLookupResult, latestRule: String, point: Point? = null): Boolean {
            ...
            ApplicationManager.getApplication().executeOnPooledThread {
                val context = ReadAction.compute<DocumentationLookupContext?, RuntimeException> {
                    ...
                    DocumentationLookupContext(result.dependency, result.versionInfo, latestRule, managedOptions, metadataPsi, originalElement)
